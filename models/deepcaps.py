...
class ConvertToCaps(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        x = squash(torch.unsqueeze(x, dim=2), dim=2) 
        return x
...
class Mask_CID(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        # ic(batch_ind.shape, max_len_indices.shape)
...
class DeepCapsModel(nn.Module):
    ...
    def __init__(self, num_class, img_height, img_width, in_channel, device):
        '''
        ...
        nn.Conv2d(self.in_channel,32,3,stride=2,padding=1),                          # 对应Primary_CapsuleLayer的输入通道数为256
        nn.ReLU(inplace=True),
        nn.BatchNorm2d(32),
        ...
        self.toCaps = ConvertToCaps()                                                                     # [64, 128, 1, 32, 32]
    ...
    def forward(self, x, target=None):
        ...
        x = self.conv1(x)                 # [64, 128, 32, 32]
        x = self.bn1(x) 
        x = self.toCaps(x)                # [64, 128, 1, 32, 32]
        ...
        # ic(x.shape)          (16, 3)
        # ic(indices.shape)    (16)
        # ic(masked.shape)     (16,1,16)

        return indices, None, x, masked
